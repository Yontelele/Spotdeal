...
const filterByBindningstid = (bindningstid: Bindningstider, tv: TvStreamingDto[]) => {
  let filteredTV = []

  switch (bindningstid) {
    case Bindningstider.INGEN_BINDNINGSTID:
      filteredTV = tv.filter((t) => t.bindningstid == 0)
      break
    case Bindningstider.TOLV_MÅNADER_BINDNINGSTID:
      filteredTV = tv.filter((t) => t.bindningstid == 12)
      break
    case Bindningstider.TJUGO_FYRA_MÅNADER_BINDNINGSTID:
      filteredTV = tv.filter((t) => t.bindningstid == 24)
      break
    default:
      return []
  }
...
export const TvRadioGroup = ({ selectedOperator }: Props) => {
  const { state, selectTvStreaming, tvStreaming } = useCartContext()
  const { selectedBindningstid, selectedTvStreaming } = state

  const bindningstidToUse = selectedOperator.id === Operatorer.TELIA ? Bindningstider.TOLV_MÅNADER_BINDNINGSTID : selectedBindningstid

  return (
    <RadioGroup value={selectedTvStreaming} onChange={(newTV) => selectTvStreaming(newTV)}>
      <div className={getGridBredbandTVRadioCards(selectedOperator.id, bindningstidToUse)}>
        {filterByBindningstid(bindningstidToUse, tvStreaming).map((bredband) => (
          <Radio
            key={bredband.id}
            value={bredband}
            className={`relative flex flex-col items-center cursor-pointer rounded-lg border bg-gray-800 p-4 shadow-sm focus:outline-hidden ${
              selectedTvStreaming?.name === bredband.name
                  ...
                  <div className={`relative flex items-center ${!checked ? "invisible" : ""}`}>
                    ...
                    <div className="relative w-10 h-6 mx-1 overflow-hidden">
                      <AnimatePresence mode="popLayout">
                        <motion.div
                          key={selectedTvStreaming?.operatorId}
