...
import BindningstidButtons from "./BindningstidButtons"
import BredbandRadioGroup from "./BredbandRadioGroup"
...
export const RegistreraBredband = () => {
  const { state, selectOperator } = useCartContext()
  const { selectedOperator, selectedBredband, cart } = state
  ...
  useEffect(() => {
    ...
    if (foundOperator) {
      if (!selectedOperator || selectedOperator.id !== foundOperator.id) {
        if (foundOperator.id !== Operatorer.HALEBOP) {
          selectOperator(foundOperator, ContractType.Bredband)
        }
      }
  ...
  function handleGetContractCodes() {
    if (!selectedBredband) return

    const cartDto = RegistreringManager.createCartRequestDto(abonnemangCart, phoneCart, selectedBredband.id, null)
  ...
  return (
    <Layout>
      {selectedOperator && (
        <HeaderCenter titel="Registrera ett nytt bredband" subtitle="Vänligen välj ett bredband">
          <Fragment>
            <BindningstidButtons operatorId={selectedOperator.id} contractType={ContractType.Bredband} />
            <BredbandRadioGroup selectedOperator={selectedOperator} />
              ...
              <div className="grid grid-cols-12 gap-6">
                        ...
                        <div className="flex items-center gap-x-3">
                          ...
                          <h2 className="min-w-0 text-sm font-semibold leading-6 text-white">
                            <div className="flex gap-x-2">
                              <span className="truncate">{selectedBredband.name}</span>
                            </div>
                          </h2>
                        </div>
                        <div className="mt-3 flex items-center gap-x-2.5 text-xs leading-5 text-gray-400">
                          <p className="truncate">Bredband</p>
                          <SmallDotIcon />
                          <p className="whitespace-nowrap">{`${selectedBredband.bindningstid} månaders bindningstid`}</p>
                          <SmallDotIcon />
                          <p className="truncate">{selectedBredband.provision}</p>
                        </div>
                      </div>
                    </div>
                  </ul>
                ...
                <div className="flex flex-col col-span-full xl:col-span-3">
                  ...
                  <button
                    ...
                    disabled={mutation.isPending || !selectedBredband}
                  >
                    {showPending ? (
...
export default RegistreraBredband
